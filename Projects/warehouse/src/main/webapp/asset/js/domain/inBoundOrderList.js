//this function is used to clear all the data in the input field when there is only one row left inside the inBoundOrder list
...
//this function is used to change and display the corresponding brandName when clicking and select a specific product in a inBoundOrderItem row
//refer line 64 to 65 in input.jsp of the inBoundOrder
$(function(){
    $(".itemProductList").change(function(){
        var $option = $(this).find("option:selected");
        var id =$option.val();
        var json = $.ajax({
            url: "/inBoundOrderItem/productData?id="+id,
            type:"POST",
            dataType: 'json',
            async: false,
            data:"",
    ...
    //this is used to add a new row in inBoundOrderList for entering a inBoundOrderItem's data
...
/*this function is used to calculate totalPrice of an inBoundOrderItem and show it on the page to prevent the user
...
//this function is used to delete an inBoundOrderItem
$(function(){
    $(".button_delete").click(function(){
        var tr = $(this).closest("tr");
        var targetId = tr.find('[tag=itemsId]').val();
        //for the case when the existing input field having data is not an existing saved inBoundOrderItem in database
        ...
        if(targetId =='0'|| targetId == '' || targetId == 'undefined' || targetId == null ){
            //For the case when there is only one input row remaining in the inBoundOrder
            if($("#edit_table_body tr").length == 1){
                //remain one input row and clean all the data in the row
                clearTrData(tr);
            }else{
                //For the case when there are more than one input row remaining in the inBoundOrder
                //just remove the new added row
                tr.remove();
            }
        }else{
            //For the case when the existing input data is an existing saved inBoundOrderItem in database
                ...
                $dialog.dialog({
                    ...
                    buttons: {
                        "Delete item": function() {
                            ...
                            $.ajax({
                                url: "/inBoundOrderItem/delete?itemId="+itemId+"&orderId="+orderId,
                                type:"POST",
                                dataType: 'json',
                                async: false,
                                data:"",
