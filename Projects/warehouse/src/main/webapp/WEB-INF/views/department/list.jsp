...
<html>
<head>
    <title>Department ListPage</title>
...
<body>
    ...
    <form:form id="searchForm" action="/department/list" method="POST" modelAttribute="qo">
        <div class="listForm">
            <div class="searchBox">
            ...
            <div id="domainList">
                <span id="domainTitle">List of Department</span>
                <c:url var="add" value="/department/input"/>
                <input type="button" value="Add a Department" class="button_redirect" data-url="<c:out value="${add}"/>"/>
            </div>
            <div class="tableResult">
                <table class="table">
                    ...
                    <tbody>
                        <c:forEach var="department" items="${pageResult.result}">
                            <tr>
                                <td><c:out value="${department.id}"/></td>
                                <td><c:out value="${department.name}"/></td>
                                <td>
                                    <c:if test="${permissionFn:checkPermission('org.construction.wms.web.controller.DepartmentController:input')}">
                                        <c:url var="edit" value="/department/input">
                                            <c:param name="id" value="${department.id}"/>
                                        </c:url>
                                        <a href="<c:out value="${edit}"/>">Edit</a>
                                    </c:if>
                                    <c:if test="${permissionFn:checkPermission('org.construction.wms.web.controller.DepartmentController:delete')}">
                                        <c:url var="delete" value="/department/delete">
                                            <c:param name="id" value="${department.id}"/>
                                        </c:url>
                                        <a href="javascript:;"  class="button_department_delete" data-url="<c:out value="${delete}"/>">Delete</a>
                                    </c:if>
                                </td>
                            </tr>
                        </c:forEach>
